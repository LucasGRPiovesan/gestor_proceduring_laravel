...
"use strict";
/*!*******************************************!*\
  !*** ./resources/js/settings/profiles.js ***!
  \*******************************************/
__webpack_require__.r(__webpack_exports__);
...
var columns = ['Profile', 'Description', 'Options'];
var storeModal = document.getElementById('exampleModal');
var updateEvent = document.getElementById('updateProfileBtn');
...
var descriptionInput = document.getElementById('profileDescription');
var current = {};
var renderTable = function renderTable(data) {
  var thead = document.getElementById('profilesTableHead');
  ...
  Array.from(document.getElementsByClassName('delete-profile')).forEach(function (button) {
    button.addEventListener('click', function () {
      var uuid = button.dataset.uuid;
      deleteProfile(uuid);
    });
...
function fetchProfile(uuid) {
  axios.get("http://localhost:8000/api/profile/".concat(uuid)).then(function (response) {
...
function updateProfile() {
  ...
  axios.patch("http://localhost:8000/api/profile/".concat(current.uuid), payload).then(function () {
    listProfiles();
    var modal = bootstrap__WEBPACK_IMPORTED_MODULE_0__.Modal.getOrCreateInstance(storeModal);
    modal.hide();
    Swal.fire({
      icon: 'success',
      title: "Profile ".concat(profileInput.value, " updated successfully!")
    });
    cleanFields();
  })["catch"](function () {
    Swal.fire({
      icon: 'error',
      title: "Error on update Profile ".concat(profileInput.value, "!")
    });
  });
}
function deleteProfile(uuid) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      axios["delete"]("http://localhost:8000/api/profile/".concat(uuid)).then(function () {
...
document.getElementById('createProfileBtn').addEventListener('click', function () {
  ...
  axios.post('http://localhost:8000/api/profile', {
    ...
    Swal.fire({
      icon: 'success',
      title: "Profile ".concat(profile, " created successfully!")
    });
    cleanFields();
  })["catch"](function () {
    Swal.fire({
      icon: 'error',
      title: "Error on create Profile ".concat(profile, "!")
    });
  });
});
function checkForChanges() {
  var nameChanged = profileInput.value !== current.profile;
  var descChanged = descriptionInput.value !== current.description;
