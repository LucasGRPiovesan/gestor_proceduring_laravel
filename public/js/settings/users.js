...
"use strict";
/*!****************************************!*\
  !*** ./resources/js/settings/users.js ***!
  \****************************************/
__webpack_require__.r(__webpack_exports__);
...
var columns = ['Name', 'Profile', 'E-mail'];
var storeModal = document.getElementById('exampleModal');
var updateEvent = document.getElementById('updateUserBtn');
...
var renderTable = function renderTable(data) {
  var thead = document.getElementById('usersTableHead');
  ...
  Array.from(document.getElementsByClassName('delete-user')).forEach(function (button) {
    button.addEventListener('click', function () {
      var uuid = button.dataset.uuid;
      deleteProfile(uuid);
    });
...
function loadProfiles() {
  axios.get('http://localhost:8000/api/profile-select').then(function (response) {
...
function fetchUser(uuid) {
  document.querySelectorAll('.only-create').forEach(function (el) {
    el.classList.add('d-none');
  });
  axios.get("http://localhost:8000/api/user/".concat(uuid)).then(function (response) {
...
function updateUser() {
  ...
  axios.patch("http://localhost:8000/api/user/".concat(current.uuid), payload).then(function () {
    listUsers();
    var modal = bootstrap__WEBPACK_IMPORTED_MODULE_0__.Modal.getOrCreateInstance(storeModal);
    modal.hide();
    Swal.fire({
      icon: 'success',
      title: "User ".concat(nameInput.value, " updated successfully!")
    });
    cleanFields();
  })["catch"](function () {
    Swal.fire({
      icon: 'error',
      title: "Error on update Profile ".concat(nameInput.value, "!")
    });
  });
}
function deleteProfile(uuid) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      axios["delete"]("http://localhost:8000/api/user/".concat(uuid)).then(function () {
...
document.getElementById('createUserBtn').addEventListener('click', function () {
  ...
  var uuid_profile = document.getElementById('profile').value;
  ...
  axios.post('http://localhost:8000/api/user', {
    ...
    Swal.fire({
      icon: 'success',
      title: "User ".concat(name, " created successfully!")
    });
    cleanFields();
  })["catch"](function () {
    Swal.fire({
      icon: 'error',
      title: "Error on create User ".concat(name, "!")
    });
  });
});
function checkForChanges() {
  var nameChanged = nameInput.value !== current.name;
  var emailChanged = emailInput.value !== current.email;
  var profileChanged = profileSelect.value !== current.uuid_profile;
  updateEvent.disabled = !(nameChanged || emailChanged || profileChanged);
}
function getMaxPage() {
  var pageLinks = pagination.querySelectorAll('[data-page]');
  return pageLinks.length;
}
function highlightCurrentPage(page) {
  pagination.querySelectorAll('.page-item').forEach(function (li) {
    return li.classList.remove('active');
  });
  var currentItem = pagination.querySelector("[data-page=\"".concat(page, "\"]"));
...
function renderPagination(current, totalPages) {
  ...
  for (var i = 1; i <= totalPages; i++) {
    ...
    li.innerHTML = "<a class=\"page-link\" href=\"#\" data-page=\"".concat(i, "\">").concat(i, "</a>");
